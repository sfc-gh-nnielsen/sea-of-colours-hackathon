"""iakabi_reaper — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``IAKABI_REAPER_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.iakabi_reaper._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.iakabi_reaper import prompt as prompt_mod
from sea_of_colours.orchestrator_2.harnesses.iakabi_reaper import (
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "IAKABI_REAPER"
THINKER_AGENT_LABEL = "IAKABI_REAPER_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``IAKABI_REAPER_SINGLE=1`` disables."""
    return os.environ.get("IAKABI_REAPER_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``IAKABI_REAPER_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("IAKABI_REAPER_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
