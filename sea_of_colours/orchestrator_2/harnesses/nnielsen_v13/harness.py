"""nnielsen_v13 — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``NNIELSEN_V13_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.nnielsen_v13._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.nnielsen_v13 import prompt as prompt_mod
from sea_of_colours.orchestrator_2.harnesses.nnielsen_v13 import (
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "NNIELSEN_V13"
THINKER_AGENT_LABEL = "NNIELSEN_V13_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``NNIELSEN_V13_SINGLE=1`` disables."""
    return os.environ.get("NNIELSEN_V13_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``NNIELSEN_V13_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("NNIELSEN_V13_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
